# 🗂️ Checkpoint SQL – Insertion de Données (DML)
...
📎 Modèle relationnel :  
👉 [Voir le schéma ici](https://i.imgur.com/q25t2MI.png)
...
📎 Données à insérer :  
👉 [Voir les données ici](https://i.imgur.com/DcLUzM5.png)

---

## 🛠️ Instructions

1. Utilisez la commande `INSERT INTO` pour insérer les données dans :
   - la table `Product` (produits),
   - la table `Customer` (clients),
   - la table `Orders` (commandes).

2. Respectez le format, les types de données, et les noms de colonnes présents dans le schéma.
